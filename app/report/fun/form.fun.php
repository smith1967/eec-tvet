...
function genInput($data, $labelW=2, $inputW=10) {
	...
	foreach($data as $k=>$row) {
		...
		if($row['type']=='wysiwyg') {
			if( !$wysiInit) {
				$systemFoot.='<script src="'.site_url('app/report/ckeditor/ckeditor.js',
				true).'"></script>';
				$systemFoot.="<script src='".site_url('asset/AdminLTE/plugins/bootstrap-wysihtml5/bootstrap3-wysihtml5.all.min.js',
				true)."'></script>";
				$wysiInit=true;
			}
 ...
 <script>$(function () {
		...
		elseif($row['type']=='sourceHL') {
			if( !$sourceHL) {
				$sourceHL=true;
				$systemFoot.='<script src="'. site_url("app/report/codemirror/lib/codemirror.js",
				true).'"></script>
 <link rel="stylesheet" href="'.
 site_url("app/report/codemirror/lib/codemirror.css",
				true).'">
 <link rel="stylesheet" href="'.
 site_url("app/report/codemirror/theme/midnight.css",
				true).'">
 <script src="'.
 site_url("app/report/codemirror/mode/javascript/javascript.js",
				true).'"></script>';
			}
			$ret.="<textarea id='".$k."' name='".$k."' ".$attr." class='".$row['class']."'>".$row['value']."</textarea>";
			$systemFoot.="
 <script>var editor".$k."=CodeMirror.fromTextArea(".$k.",
				{
				lineNumbers: true
			}
			);
 ...
 <i class="fa fa-search"></i>ค้นหา</button></span>';
 $systemFoot.='
 <script>$(\'#'.$k.'_btn\').click(function(e){
 e.preventDefault();
			$(\'#'.$k.'\').click();
...
elseif($row['type']=='select') {
	$ret.="<select id='".$k."' name='".$k."' ".$attr." class='".$row['class']."'>".gen_option($row['item'],
	$row['def'])."</select>";
}
...
if(isset($row['icon']))$ret.='
 <div class="input-group-addon"><i class="'.$row['icon'].'"></i></div>';
 $ret.="</div></div>";
if (strpos($row['class'],
'flat-red') !==false) {
	icheck();
...
<ul class="nav nav-tabs">'.$tab_pane_nav.' </ul><div class="tab-content">'.$ret.' </div></div>';
...
function genForm($data) {
 ...
 $bindingInput='';
			...
			foreach($data['ajaxSubmit'] as $inputName=>$detail) {
				...
				$inputNo++; //$('textarea#mytextarea').val();
...
function icheck() {
	...
	if( !$called) {
		$calles=1;
		$icheck_url=site_url('asset/AdminLTE/plugins/iCheck/icheck.min.js',
		true);
		$icheckCssURL=site_url('asset/AdminLTE/plugins/iCheck/all.css',
		true);
		$systemFoot.="<script src='".$icheck_url."'></script>";
		$systemFoot.="
 <script>$('input[type=\"checkbox\"].flat-red, input[type=\"radio\"].flat-red').iCheck( {
			checkboxClass: 'icheckbox_flat-blue', radioClass: 'iradio_flat-blue'
		}
		);
		</script><link rel=\"stylesheet\" href=\"".$icheckCssURL."\">
 ";
