...
$content_header = array(
    'header' => 'ความต้องนักศึกษาระบบทวิภาคี',
    'subheader' => 'แสดง',
    'breadcrumb' => 'ความต้องนักศึกษาระบบทวิภาคี'
    );
...
<?php
function get_req_dvt($business_id) {
    global $db;
//req_dvt : req_id     business_id    major_id    level  male     female  common    spacial_condition  training_start   training_end
     $query = "SELECT * FROM req_dvt WHERE business_id=" . pq($business_id) . " ORDER BY req_id DESC";
...
function count_req_dvt($business_id) {
    global $db;
//    $start = $page * $limit;
//    SELECT * FROM `industrial_group`
     $query = "SELECT * FROM req_dvt WHERE business_id=" . pq($business_id) . "";
    $result = mysqli_query($db, $query);
    if (!$result) {
    printf("Error: %s\n", mysqli_error($db));
    exit();
