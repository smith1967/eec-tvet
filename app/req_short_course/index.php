...
$content_header = array(
    'header' => 'สมัครเข้ารับการอบรมหลักสูตรระยะสั้น',
    'subheader' => 'แสดง',
    'breadcrumb' => 'สมัครเข้ารับการอบรมหลักสูตรระยะสั้น'
    );
...
<?php
function get_req_shortcourses($business_id) {
    global $db;
//    $start = $page * $limit;
//    SELECT * FROM `industrial_group`
     $query = "SELECT * FROM req_shortcourses WHERE business_id=" . pq($business_id) . "";
...
function count_req_shortcourses($business_id) {
    global $db;
//    $start = $page * $limit;
//    SELECT * FROM `industrial_group`
     $query = "SELECT * FROM req_shortcourses WHERE business_id=" . pq($business_id) . "";
...
function get_shortcourse_name($shortcourse_code) {
    global $db;
    $query = "SELECT * FROM shortcourses WHERE  shortcourse_code=" . pq($shortcourse_code) ."  " ;
    $result = mysqli_query($db, $query);
    if (!$result) {
    printf("Error: %s\n", mysqli_error($db));
    exit();
